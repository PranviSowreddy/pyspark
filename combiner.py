...
def graph_metrics(edges_rdd):
    ...
    '''with open("outputs1.txt","w") as outfile:
...
def cycle_detection(edges_rdd, sc, max_len=25):
    ...
    # Build adjacency list with combineByKey for better performance
    ...
    for length in range(2, max_len + 1):
        # Get adjacency list from broadcast
        adj = adj_list_bc.value
        
        # Join paths with adjacency list
        joined = (
        ...
        # Separate cycles from ongoing paths
        ...
        # Early termination if no more paths
        if paths.isEmpty():
            paths.unpersist()
            break
...
def cycle_metrics(sorted_cycles_list,process_involved):
    for c in sorted_cycles_list:
        for n in c:
            if n.startswith("P"):
                process_involved.add(n)    
    '''with open("outputs1.txt","a") as outfile:
...
def main(log_file,cores=None):
    conf=SparkConf().setAppName("RAG_Deadlock_Analysis_WITH_COMBINER")
    if cores:
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python with_combiner.py <log_file_path> [num_cores]")
        sys.exit(1)
    log_file = sys.argv[1]
